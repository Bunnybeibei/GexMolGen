...
def make_cuda(tensors):
    ...
    tree_tensors = [make_tensor(x).cuda() for x in tree_tensors[:-1]] + [tree_tensors[-1]]
...
class HierVAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.encoder = HierMPNEncoder(args.vocab_drug, args.atom_vocab, args.rnn_type, args.embed_size, args.hidden_size, args.depthT, args.depthG, args.dropout_drug)
        self.decoder = HierMPNDecoder(args.vocab_drug, args.atom_vocab, args.rnn_type, args.embed_size, args.hidden_size, args.latent_size, args.diterT, args.diterG, args.dropout_drug)
        ...
        self.R_mean_bn = nn.BatchNorm1d(num_features=args.latent_size)

    def rsample(self, z_vecs, W_mean, W_var, perturb=True, is_bn=True):
        batch_size = z_vecs.size(0)
        z_mean = W_mean(z_vecs)

        if is_bn:
            z_mean = self.R_mean_bn(z_mean)
    ...
    def rsample2(self, z_vecs, W_mean, W_var, perturb_z=True, is_bn=True):
        batch_size = z_vecs.size(0)
        z_mean = W_mean(z_vecs)
        ...
        kl_loss = -0.5 * torch.sum(1.0 + z_log_var - z_mean * z_mean - torch.exp(z_log_var)) / batch_size
        epsilon = torch.randn_like(z_mean).cuda()
        z_vecs = z_mean + torch.exp(z_log_var / 2) * epsilon if perturb_z else z_mean
        return z_vecs, kl_loss

    def sample(self, root_vecs, greedy):
        root_vecs = self.R_mean2(root_vecs)
        root_vecs = self.R_mean_bn(root_vecs)
        return self.decoder.decode((root_vecs, root_vecs, root_vecs), greedy=greedy, max_decode_step=100)

    def reconstruct(self, graphs, tensors, orders):
    ...
    def finetune(self, graphs, tensors, orders, root_vecs, beta):
        tree_tensors, graph_tensors = tensors = make_cuda(tensors)
        # root_vecs = torch.from_numpy(root_vecs).cuda().float()
        _, tree_vecs, _, graph_vecs = self.encoder(tree_tensors, graph_tensors)
        root_vecs, root_kl = self.rsample2(root_vecs, self.R_mean2, self.R_var2, perturb_z=True)
        kl_div = root_kl
    ...
    def latent(self,graphs, tensors, orders, perturb_z=False):
        tree_tensors, graph_tensors = tensors = make_cuda(tensors)

        root_vecs, tree_vecs, _, graph_vecs = self.encoder(tree_tensors, graph_tensors)
        root_vecs, root_kl = self.rsample(root_vecs, self.R_mean, self.R_var, perturb_z, False)
        return root_vecs
...
class HierVGNN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.decoder = HierMPNDecoder(args.vocab, args.atom_vocab, args.rnn_type, args.embed_size, args.hidden_size, args.latent_size, args.diterT, args.diterG, args.dropout, attention=True)
        self.encoder.tie_embedding(self.decoder.hmpn)
