...
class MolGraph(object):
    ...
    def label_tree(self):
        ...
        for i,cls in enumerate(self.clusters):
            ...
            tree.nodes[i]['ismiles'] = ismiles = get_smiles(cmol)#集团的化学形式
            tree.nodes[i]['inter_label'] = inter_label
            tree.nodes[i]['smiles'] = smiles = get_smiles(set_atommap(cmol))# 除了H原子以外的原子，可以重复出现
    ...
    def tensorize(mol_batch, vocab, avocab):
        ...
        cgraph = torch.zeros(len(tree_batchG) + 1, max_cls_size).int() # 设置cluster的one-hot编码，大小(32个分子的motif_layer节点数，clusters的种类)
        for v,attr in tree_batchG.nodes(data=True):
            ...
            tree_batchG.nodes[v]['cluster'] = cls = [x + offset for x in attr['cluster']]# 映射到整合后的原子序号上
        ...
        tree_tensors = tree_tensors[:4] + (cgraph, tree_scope) # (fnode, fmess, agraph, bgraph, cgraph, tree_scope) 增加了类似[cls]的前置项
        return (tree_batchG, graph_batchG), (tree_tensors, graph_tensors), all_orders

    @staticmethod
    def tensorize_graph(graph_batch, vocab):# 因为是批次训练，要将32种分子放到一批张量中所以要区分
        ...
        for bid,G in enumerate(graph_batch):
            offset = len(fnode)
            scope.append( (offset, len(G)) ) # 把几种分子图分开，前者是他们的开始序号，后者是单个分子包含的node数
            G = nx.convert_node_labels_to_integers(G, first_label=offset) # 每张图的序号不再从1开始而是从它开始的序号
            all_G.append(G)
            fnode.extend( [None for v in G.nodes] ) #分配与分子图节点数量相等的内存

            for v, attr in G.nodes(data='label'):
                G.nodes[v]['batch_id'] = bid # 每个分子图所属batch的标识
                fnode[v] = vocab[attr] # fnode的成分(原子类型（可以重复)， 集团化学式）输入是化学式，但其实是字典查找
                agraph.append([])
            ...
            for u, v in G.edges:
                eid = edge_dict[(u, v)] # edge_dict是边的序号
                for w in G.predecessors(u): # predecessors指的是指向u的母节点（与(u,v)相邻的edge，不包括(u,v))
                    if w == v: continue
                    bgraph[eid].append( edge_dict[(w, u)] )

        fnode[0] = fnode[1] # 把None去掉
        fnode = torch.IntTensor(fnode) # fnode的成分(原子类型（可以重复)， 集团化学式）输入是化学式，但其实是字典查找
        fmess = torch.IntTensor(fmess) # 边的整合信息(起点， 终点， 不知道啥信息， 0)
        agraph = create_pad_tensor(agraph) # 第N个原子连接的边的序号，max_len表示这些原子中连接边最多的数
        bgraph = create_pad_tensor(bgraph) # 第N个边顺流过来的序号，max_len表示这些边中顺流边最多的数
...
if __name__ == "__main__":
    ...
    for s in test_smiles: #sys.stdin:#test_smiles:
        print(s.strip("\r\n "))
