...
class HierMPNDecoder(nn.Module):
    ...
    def init_decoder_state(self, tree_batch, tree_tensors, src_root_vecs):
        ...
        htree = HTuple()
        htree.mess = self.rnn_cell.get_init_state(tree_tensors[1], src_root_vecs) #src_root_vecs (batch_size, hidden_size) 起作用
    ...
    def forward(self, src_mol_vecs, graphs, tensors, orders):
        ...
        src_root_vecs, src_tree_vecs, src_graph_vecs = src_mol_vecs # (batch, latent_size), (batch, latent_size), (batch, latent_size)
        init_vecs = src_root_vecs if self.latent_size == self.hidden_size else self.W_root(src_root_vecs) # (batch, hidden_size)
        ...
        for t in range(maxt):
            ...
            subtree = htree.emask.new_tensor(subtree[0],dtype=torch.int64), \
                      htree.emask.new_tensor(subtree[1],dtype=torch.int64)
            ...
            new_atoms = []
            for i in batch_list:
                # topo_vecs, batch_idx, topo_labels
                xid, yid, tlab = orders[i][t]
