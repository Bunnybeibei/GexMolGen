...
class IncTree(IncBase):
    ...
    def __init__(self, batch_size, node_fdim, edge_fdim, max_nodes=170, max_edges=200, max_nb=12, max_sub_nodes=20):
    ...
    def register_cgraph(self, i, nodes, edges, attached):
        try:self.cgraph[i, :len(nodes)] = self.fnode.new_tensor(nodes)
        except:
                pass
...
class IncGraph(IncBase):
    ...
    def __init__(self, avocab, batch_size, node_fdim, edge_fdim, max_nodes=170, max_edges=300, max_nb=10):
    ...
    def get_mol(self):
        ...
        mol_smile_list = [None] * len(self.batch)
        for batch_idx, batch_atoms in self.batch.items():
            ...
            if mol is None: 
                ...
                mol_smile_list[batch_idx] = None
            else:
                for atom in mol.GetAtoms():
                    atom.SetAtomMapNum(0)
                # 更改
                mol_list[batch_idx] = mol
                mol_smile_list[batch_idx] = Chem.MolToSmiles(mol)
        return mol_list, mol_smile_list
